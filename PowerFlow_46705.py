...
def calculate_F(Ybus,Sbus,V,pv_index,pq_index):
    """
    ToDo: Calculate the difference between specified value and calculated value
    """
    Delta_S = Sbus - V * (Ybus.dot(V)).conj()
    Delta_P = np.real(Delta_S)
    Delta_Q = np.imag(Delta_S)
    """
    ToDo: Construct F to have the right structure of the indexes of different busses.
    """
    F = F = np.concatenate((Delta_P[pv_index],Delta_P[pq_index], Delta_Q[pq_index]),axis=0)
    return F
...
def generate_Derivatives(Ybus,V):
    ...
    """
    ToDo:  Generate the derivatives used in the Jacobian
    """
    J_ds_dVm = np.diag(V/np.absolute(V)).dot(np.diag((Ybus.dot(V)).conj()))+np.diag(V).dot(Ybus.dot(np.diag(V/np.absolute(V))).conj())
    J_dS_dTheta = J_dS_dTheta = 1j*np.diag(V).dot((np.diag(Ybus.dot(V))-Ybus.dot(np.diag(V))).conj()) 
    return J_ds_dVm,J_dS_dTheta


# 5. the generate_Jacobian() function
def generate_Jacobian(J_dS_dVm,J_dS_dTheta,pv_index,pq_index):
    # Construct the Jacobian using the output of the generate_Derivatives() fct     
    pvpq_ind = np.append(pv_index,pq_index)
    
    """
     ToDo: Construct Jacobian from the indexes of the different nodes
    """
...
def Update_Voltages(dx,V,pv_index,pq_index):
    ...
    V = Vm * np.exp(1j*Theta) # convert into complex voltage
...
def DisplayResults_and_loading(V,lnd):    
    ...
    """
    ToDo: Determin branch flows and branch loading
    Suggested approach: 
        Use Y_to and Y_fr matrices to determine branch currents
        Determine S
        Calculate percentage apparent power loading comparing calculated MVA flow with nominal MVA rating (br_MVA). 
        Be aware that the power flow calculations are performed in pu on the specified system base (MVA_base) while br_MVA is specified in MVA 
    """    
    S_to = V[br_t]*(Y_to.dot(V)).conj() 
    to_loading = np.absolute(S_to)*MVA_base / br_MVA*100 
    P_to = np.real(S_to)
    Q_to = np.imag(S_to)   
    
    # Power from each bus by branch 
    S_from = V[br_f]*(Y_from.dot(V)).conj() 
    fr_loading = np.absolute(S_from)*MVA_base / br_MVA*100
    P_from = np.real(S_from)
    Q_from = np.imag(S_from)
    
    # Power injected at each bus
    S_inj = S_inj = V*(Ybus.dot(V)).conj()
    P_inj = np.real(S_inj)
    Q_inj = np.imag(S_inj)
...
####################################################
#  Determine system violations and print them to the console   #
####################################################
...
def System_violations(V,lnd):
    ...
    Y_from = lnd.Y_fr; Y_to = lnd.Y_to;  # Y_from,Y_to = the admittance matrices used to determine the branch flows

    #line flows and generator injections:
    """
    ToDo: Calculate Injections and branch flows (same as in DisplayResults_and_loading())
    """
    ...
    for i,fr,to,MVA in zip(range(len(br_f)),br_f,br_t,br_MVA):
        fr_loading = np.absolute(S_from[i])*lnd.MVA_base/ MVA   
        to_loading = np.absolute(S_to[i])*lnd.MVA_base/ MVA
