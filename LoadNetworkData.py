...
def LoadNetworkData(filename):
    ...
    for line,i in zip(line_data,range(len(line_data))):
        ...
        """
        ToDO: Formulate 2x2 component admittance matrix of line/cable pi equivalent
        """
        yser = 1/(R+1j*X)
        ysh = 1j*B/2
        Yps_mat[0,0] = yser+ysh  
        Yps_mat[0,1] = -yser
        Yps_mat[1,0] = -yser         
        Yps_mat[1,1] = yser+ysh
        
        # Update the bus admittance matrix
        """
        ToDo: Add the component admittance matrix to the correct locations in the existing Ybus
        """
        ind_Ybus = [ind_fr, ind_to]
    ...
    for line,i in zip(tran_data,range(len(line_data),M_branches)):
        ...
        Yps_mat = np.zeros((2,2),dtype=complex)
        """
        ToDO: Formulate 2x2 component admittance matrix of transformers
        """
        
        Ye = 1 / (R + 1j * X)
        # n è il modulo, ang1 è lo sfasamento in gradi (converti in radianti)
        as_val = n * np.cos(np.radians(ang1)) 
        bs_val = n * np.sin(np.radians(ang1))
        n_sq = as_val**2 + bs_val**2  # Questo è |n|^2
        
        
        Yps_mat[0,0] = Ye/(n_sq)  
        Yps_mat[0,1] = -Ye/(as_val-1j*bs_val)
        Yps_mat[1,0] = -Ye/(as_val+1j*bs_val)           
        Yps_mat[1,1] = Ye
